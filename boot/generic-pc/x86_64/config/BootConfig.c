...
static EFI_GUID gTomatBootConfigGuid = { 0x2714d689,
					 0x1da6,
					 0x49d3,
					 { 0x9b, 0x82, 0xa9, 0xdf, 0x7a, 0xe1,
					   0xb8, 0x25 } };

static CHAR16 *gTomatBootConfigName = L"TomatBoot";

BOOT_CONFIG gBootConfigOverride = { .BootDelay = -1,
				    .DefaultOS = -1,
				    .GfxMode = -1,
				    .OverrideGfx = FALSE };
...
void LoadBootConfig(BOOT_CONFIG *config)
{
	...
	EFI_STATUS Status = gRT->GetVariable(gTomatBootConfigName,
					     &gTomatBootConfigGuid, &Attributes,
					     &Size, config);
...
{
	ASSERT_EFI_ERROR(gRT->SetVariable(
		gTomatBootConfigName, &gTomatBootConfigGuid,

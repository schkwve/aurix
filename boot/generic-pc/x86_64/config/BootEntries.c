...
{
	int i = 0;
	for (LIST_ENTRY *Link = gBootEntries.ForwardLink; Link != &gBootEntries;
	     Link = Link->ForwardLink, i++) {
		if (index == i) {
...
static EFI_STATUS ParseUri(CHAR16 *Uri, EFI_SIMPLE_FILE_SYSTEM_PROTOCOL **OutFs,
			   CHAR16 **OutPath)
{
	...
	*OutPath = CopyString(Path);
	...
	if (StrCmp(Uri, L"boot") == 0) {
		...
		if (*Root == L'\0') {
			...
			BootDevicePath =
				RemoveLastDevicePathNode(BootDevicePath);
			CHECK(BootDevicePath != NULL);

			CHAR16 *Text = ConvertDevicePathToText(BootDevicePath,
							       TRUE, TRUE);
			TRACE("Boot device path: %s", Text);
			FreePool(Text);

			// iterate the protocols
			EFI_CHECK(gBS->LocateHandleBuffer(
			...
			CHECK_TRACE(index != -1,
				    "Could not find partition number `%d`",
				    PartNum);
		...
		for (int i = 0; i < HandleCount; i++) {
			...
			if (Hd->SignatureType == SIGNATURE_TYPE_GUID &&
			    CompareGuid(&Guid, (EFI_GUID *)Hd->Signature)) {
				// we freaking found it!
				index = i;
				break;
			}
		...
		// make sure we found it
		CHECK_TRACE(index != -1,
			    "Could not find partition or fs with guid of `%s`",
			    Root);

		// get the fs from the guid
		EFI_CHECK(gBS->HandleProtocol(Handles[index],
					      &gEfiSimpleFileSystemProtocolGuid,
					      (void **)OutFs));
	} else {
...
static EFI_STATUS LoadBootEntries(EFI_SIMPLE_FILE_SYSTEM_PROTOCOL *FS,
				  LIST_ENTRY *Head)
{
	EFI_STATUS Status = EFI_SUCCESS;
	...
	for (int i = 0; i < ARRAY_SIZE(ConfigPaths); i++) {
		if (!EFI_ERROR(root->Open(root, &file, ConfigPaths[i],
	...
	*Head = (LIST_ENTRY)INITIALIZE_LIST_HEAD_VARIABLE(*Head);
	...
	while (TRUE) {
		...
		EFI_CHECK(FileHandleReadLine(file, Line, &LineSize, FALSE,
					     &Ascii));
		...
		if (Line[0] == L':') {
			...
			CurrentEntry->BootModules =
				(LIST_ENTRY)INITIALIZE_LIST_HEAD_VARIABLE(
					CurrentEntry->BootModules);
			...
			if (CHECK_OPTION(L"PATH") ||
				...
				if (StrCmp(Protocol, L"linux") == 0) {
					...
					CHECK_FAIL_TRACE(
						"Unknown protocol `%s` for option `%s`",
				...
				BOOT_MODULE *Module =
					AllocateZeroPool(sizeof(BOOT_MODULE));
				Module->Fs = FS;
				Module->Tag = L"";
				CHECK_AND_RETHROW(ParseUri(Path, &Module->Fs,
							   &Module->Path));
				InsertTailList(&CurrentEntry->BootModules,
					       &Module->Link);
				...
				// next
				if (IsNodeAtEnd(&CurrentEntry->BootModules,
						&CurrentModuleString->Link)) {
					CurrentModuleString = NULL;
				} else {
					CurrentModuleString = BASE_CR(
						GetNextNode(
							&CurrentEntry
								 ->BootModules,
							&CurrentModuleString
...
EFI_STATUS GetBootEntries(LIST_ENTRY *Head)
{
	...
	EFI_CHECK(gBS->HandleProtocol(gImageHandle,
				      &gEfiLoadedImageProtocolGuid,
				      (void **)&LoadedImage));
	EFI_CHECK(gBS->HandleProtocol(LoadedImage->DeviceHandle,
				      &gEfiDevicePathProtocolGuid,
				      (void **)&BootDevicePath));

	// locate the file system
	EFI_CHECK(gBS->LocateDevicePath(&gEfiSimpleFileSystemProtocolGuid,
					&BootDevicePath, &FsHandle));
	EFI_CHECK(gBS->HandleProtocol(
		FsHandle, &gEfiSimpleFileSystemProtocolGuid, (void **)&BootFs));

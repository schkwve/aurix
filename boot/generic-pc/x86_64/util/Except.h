...
#define CHECK_ERROR_LABEL_TRACE(expr, error, label, fmt, ...)       \
	do {                                                        \
		if (!(expr)) {                                      \
			Status = error;                             \
			ERROR("%r at %a (%a:%d)", Status, __func__, \
...
#define CHECK_ERROR_LABEL(expr, error, label) \
	CHECK_ERROR_LABEL_TRACE(expr, error, label, "")
#define CHECK_ERROR_TRACE(expr, error, fmt, ...) \
	CHECK_ERROR_LABEL_TRACE(expr, error, cleanup, fmt, ##__VA_ARGS__)
#define CHECK_ERROR(expr, error) \
	CHECK_ERROR_LABEL_TRACE(expr, error, cleanup, "")
#define CHECK_TRACE(expr, fmt, ...) \
	CHECK_ERROR_TRACE(expr, EFI_INVALID_PARAMETER, fmt, ##__VA_ARGS__)
...
#define CHECK_FAIL_TRACE(fmt, ...) \
	CHECK_ERROR_TRACE(0, EFI_INVALID_PARAMETER, fmt, ##__VA_ARGS__)
...
#define CHECK_AND_RETHROW_LABEL(error, label)                       \
	do {                                                        \
		Status = error;                                     \
		if (EFI_ERROR(Status)) {                            \
			ERROR("  rethrown at %a (%a:%d)", __func__, \
			      __FILENAME__, __LINE__);              \
			goto label;                                 \
		}                                                   \
	} while (0)
...
#define WARN_ON(expr, fmt, ...)                                            \
	do {                                                               \
		if (expr) {                                                \
			WARN("Warning! " fmt " at (%a:%d)", ##__VA_ARGS__, \

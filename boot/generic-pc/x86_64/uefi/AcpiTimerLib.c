...
EFI_STATUS AcpiTimerLibConstructor()
{
	...
	EFI_ACPI_1_0_FIXED_ACPI_DESCRIPTION_TABLE *Facp = GetAcpiTable(
...
{
	InternalAcpiDelay((UINT32)DivU64x32(
		MultU64x32(MicroSeconds, ACPI_TIMER_FREQUENCY), 1000000u));
	return MicroSeconds;
}
...
{
	InternalAcpiDelay((UINT32)DivU64x32(
		MultU64x32(NanoSeconds, ACPI_TIMER_FREQUENCY), 1000000000u));
	return NanoSeconds;
}
...
UINT64
EFIAPI
GetPerformanceCounterProperties(OUT UINT64 *StartValue,
...
GetTimeInNanoSecond(IN UINT64 Ticks)
{
	...
	//        Frequency
	//
	NanoSeconds = MultU64x32(DivU64x32Remainder(Ticks, ACPI_TIMER_FREQUENCY,
						    &Remainder),
				 1000000000u);

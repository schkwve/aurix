...
static void draw()
{
	...
	FillBox(0, (int)(height - 2), (int)width, 1,
...
MENU EnterBootMenu()
{
	...
	while (TRUE) {
		...
		for (LIST_ENTRY *link = gBootEntries.ForwardLink;
			...
			// write the option
			WriteAt(6, 2 + i, "%s (%s) - %a", entry->Name,
				entry->Path, loader_names[entry->Protocol]);
		}

		// draw the shutdown option
		if (i == selected) {
			FillBox(4, (int)(2 + i + 1), (int)width - 8, 1,
				EFI_TEXT_ATTR(EFI_BLACK, EFI_LIGHTGRAY));
		} else {
			FillBox(4, (int)(2 + i + 1), (int)width - 8, 1,

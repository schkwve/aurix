	...
	do {                                                              \
		if (selected == control_line) {                           \
			gST->ConOut->SetAttribute(                        \
				gST->ConOut,                              \
				EFI_TEXT_ATTR(EFI_WHITE, EFI_LIGHTGRAY)); \
			__VA_ARGS__;                                      \
		} else {                                                  \
			gST->ConOut->SetAttribute(                        \
				gST->ConOut,                              \
...
MENU EnterSetupMenu()
{
	...
	do {
		...
		WriteAt(controls_start, control_line++, "Boot Delay: %d",
			config.BootDelay);
		...
		ASSERT_EFI_ERROR(gop->QueryMode(gop, config.GfxMode,
						&sizeOfInfo, &info));
		WriteAt(controls_start, control_line++,
			"Graphics Mode: %dx%d (BGRA8)",
		...
		WriteAt(controls_start, control_line++, "Override Graphics: %a",
			config.OverrideGfx ? "True" : "False");

...
void NORETURN JumpToStivale2Kernel(STIVALE2_STRUCT *strct, UINT64 Stack,
				   void *KernelEntry, BOOLEAN level5);

static EFI_STATUS LoadStivaleHeader(EFI_SIMPLE_FILE_SYSTEM_PROTOCOL *FS,
				    CHAR16 *file, STIVALE2_HEADER *header,
				    BOOLEAN *HigherHalf)
{
	...
	*HigherHalf = FALSE;
	...
	Elf64_Shdr shstr;
	CHECK_AND_RETHROW(
		FileRead(image, &shstr, sizeof(Elf64_Shdr),
			 ehdr.e_shoff + ehdr.e_shentsize * ehdr.e_shstrndx));
	names = AllocatePool(shstr.sh_size);
	CHECK_AND_RETHROW(
		FileRead(image, names, shstr.sh_size, shstr.sh_offset));
	...
	for (int i = 0; i < ehdr.e_shnum; i++) {
		CHECK_AND_RETHROW(
			FileRead(image, &shdr, sizeof(Elf64_Shdr),
				 ehdr.e_shoff + ehdr.e_shentsize * i));
	...
	CHECK_AND_RETHROW(
		FileRead(image, header, sizeof(*header), shdr.sh_offset));
...
EFI_STATUS LoadStivale2Kernel(BOOT_ENTRY *Entry)
{
	...
	CHECK_AND_RETHROW(LoadStivaleHeader(Entry->Fs, Entry->Path, &Header,
					    &HigherHalf));
	if (HigherHalf) {
	...
	WARN_ON(Header.Flags & STIVALE2_HEADER_FLAG_KASLR,
		"KASLR Is not supported yet");
	...
	// iterate the tags
	STIVALE2_HDR_TAG *Tag = Header.Tags > (void *)0xffffffff80000000 ?
					Header.Tags - 0xffffffff80000000 :
					Header.Tags;
	while (Tag) {
		...
		Tag = Tag->Next > (void *)0xffffffff80000000 ?
			      Tag->Next - 0xffffffff80000000 :
			      Tag->Next;
	}
	...
	WARN_ON(FramebufferReq == NULL,
		"Text mode is not supported, forcing graphics mode");
	...
	if (!config.OverrideGfx && FramebufferReq != NULL &&
	    FramebufferReq->FramebufferWidth != 0 &&
	    FramebufferReq->FramebufferHeight != 0) {
		GfxMode = GetBestGfxMode(FramebufferReq->FramebufferWidth,
					 FramebufferReq->FramebufferHeight);
	}

	// set graphics mode
	EFI_GRAPHICS_OUTPUT_PROTOCOL *gop = NULL;
	ASSERT_EFI_ERROR(gBS->LocateProtocol(&gEfiGraphicsOutputProtocolGuid,
					     NULL, (VOID **)&gop));
	...
	STIVALE2_STRUCT_TAG_CMDLINE *Cmdline =
		AllocateZeroPool(sizeof(STIVALE2_STRUCT_TAG_CMDLINE));
	...
	STIVALE2_STRUCT_TAG_FRAMEBUFFER *Framebuffer =
		AllocateZeroPool(sizeof(STIVALE2_STRUCT_TAG_FRAMEBUFFER));
	...
	void *AcpiTable = NULL;
	if (!EFI_ERROR(EfiGetSystemConfigurationTable(&gEfiAcpi20TableGuid,
						      &AcpiTable))) {
		STIVALE2_STRUCT_TAG_RSDP *Rsdp =
			AllocateZeroPool(sizeof(STIVALE2_STRUCT_TAG_RSDP));
		EFI_ACPI_2_0_ROOT_SYSTEM_DESCRIPTION_POINTER *ActualRsdp =
			AcpiTable;
		Rsdp->Identifier = STIVALE2_STRUCT_TAG_RSDP_IDENT;
		Rsdp->Rsdp = AllocateReservedCopyPool(ActualRsdp->Length,
						      ActualRsdp);
		*Next = Rsdp;
		Next = &Rsdp->Next;
	} else if (!EFI_ERROR(EfiGetSystemConfigurationTable(
			   &gEfiAcpi10TableGuid, &AcpiTable))) {
		STIVALE2_STRUCT_TAG_RSDP *Rsdp =
			AllocateZeroPool(sizeof(STIVALE2_STRUCT_TAG_RSDP));
		Rsdp->Identifier = STIVALE2_STRUCT_TAG_RSDP_IDENT;
		Rsdp->Rsdp = AllocateReservedCopyPool(
	...
	STIVALE2_STRUCT_TAG_FIRMWARE *Firmware =
		AllocateZeroPool(sizeof(STIVALE2_STRUCT_TAG_FIRMWARE));
	...
	*Next = Firmware;
	...
	TRACE("Setting epoch");
	STIVALE2_STRUCT_TAG_EPOCH *Epoch =
		AllocateZeroPool(sizeof(STIVALE2_STRUCT_TAG_EPOCH));
	Epoch->Identifier = STIVALE2_STRUCT_TAG_EPOCH_IDENT;
	EFI_TIME Time = { 0 };
	EFI_CHECK(gRT->GetTime(&Time, NULL));
	Epoch->Epoch = GetUnixEpoch(Time.Second, Time.Minute, Time.Hour,
	...
	if (!IsListEmpty(&Entry->BootModules)) {
		...
		for (LIST_ENTRY *Link = GetFirstNode(&Entry->BootModules);
		     Link != &Entry->BootModules; Link = Link->ForwardLink) {
			ModulesCount++;
		}

		Modules = AllocateZeroPool(sizeof(STIVALE2_STRUCT_TAG_MODULES) +
					   sizeof(STIVALE2_MODULE) *
						   ModulesCount);
		...
		UINTN Index = 0;
		for (LIST_ENTRY *Link = Entry->BootModules.ForwardLink;
		     Link != &Entry->BootModules;
		     Link = Link->ForwardLink, Index++) {
			BOOT_MODULE *Module = BASE_CR(Link, BOOT_MODULE, Link);
			UINTN Start = 0;
			UINTN Size = 0;
			CHECK_AND_RETHROW(
				LoadBootModule(Module, &Start, &Size));

			STIVALE2_MODULE *NewModule = &Modules->Modules[Index];
			NewModule->Begin = Start;
			NewModule->End = Start + Size;
			UnicodeStrToAsciiStrS(Module->Tag, NewModule->String,
					      sizeof(NewModule->String));
			TRACE("    Added %s (%s) -> %p - %p", Module->Tag,
			      Module->Path, Start, Start + Size);
		}
	}
	...
	if (RequestedSmp) {
		...
		EFI_CHECK_LABEL(gBS->AllocatePages(
					AllocateMaxAddress, EfiBootServicesCode,
					EFI_SIZE_TO_PAGES(gSmpTrampolineEnd -
							  gSmpTrampoline),
					&SmpTplBase),
				invalid_cpu_info);
		...
		EFI_CHECK_LABEL(gBS->AllocatePages(AllocateMaxAddress,
						   EfiBootServicesData,
						   gGdtPtr.Limit + 1, &NewGdt),
				invalid_cpu_info);
		CopyMem((void *)NewGdt, (void *)gGdtPtr.Base,
			gGdtPtr.Limit + 1);
		gGdtPtr.Base = NewGdt;
		...
		EFI_ACPI_1_0_MULTIPLE_APIC_DESCRIPTION_TABLE_HEADER *Madt =
			GetAcpiTable(EFI_ACPI_1_0_APIC_SIGNATURE);
		CHECK_ERROR_LABEL(Madt != NULL, EFI_NOT_FOUND,
				  invalid_cpu_info);
		...
		UINTN CpuCount = 0;
		for (UINT8 *MadtEntry = MadtEntries;
		     MadtEntry <
		     MadtEntries +
			     (Madt->Header.Length -
			      sizeof(EFI_ACPI_1_0_MULTIPLE_APIC_DESCRIPTION_TABLE_HEADER));
		...
		Smp = AllocateZeroPool(sizeof(STIVALE2_STRUCT_TAG_SMP) +
				       sizeof(STIVALE2_SMP_INFO) * CpuCount);
		Smp->Identifier = STIVALE2_STRUCT_TAG_SMP_IDENT;
		Smp->CpuCount = 0;
		Smp->Flags = GetApicMode() == Requestedx2Apic ?
				     STIVALE2_HEADER_TAG_SMP_FLAG_X2APIC :
				     0;
				...
				if (!Requestedx2Apic) {
					...
					*Lapic = (void *)MadtEntry;
					...
					if (!(Lapic->Flags &
						...
						if ((Madt->Header.Revision <
						     EFI_ACPI_6_3_MULTIPLE_APIC_DESCRIPTION_TABLE_REVISION) ||
						    !(Lapic->Flags &
						      EFI_ACPI_6_3_LOCAL_APIC_ONLINE_CAPABLE)) {
							continue;
						}
					}
...
got_cpu_info:
	...
	CHECK(gBS->GetMemoryMap(&MemoryMapSize,
				(EFI_MEMORY_DESCRIPTOR *)TmpMemoryMap, &MapKey,
				&DescriptorSize,
				&DescriptorVersion) == EFI_BUFFER_TOO_SMALL);
	...
	STIVALE2_STRUCT_TAG_MEMMAP *Memmap = AllocateZeroPool(
		sizeof(STIVALE2_STRUCT_TAG_MEMMAP) +
		(MemoryMapSize / DescriptorSize) * sizeof(STIVALE2_MMAP_ENTRY));
	Memmap->Identifier = STIVALE2_STRUCT_TAG_MEMMAP_IDENT;
	STIVALE2_MMAP_ENTRY *StartFrom = Memmap->Memmap;
	*Next = Memmap;

	// call it
	EFI_CHECK(gBS->GetMemoryMap(&MemoryMapSize, MemoryMap, &MapKey,
	...
	if (RequestedSmp) {
		...
		MemoryFence();
		CopyMem((void *)SmpTplBase, gSmpTrampoline,
			gSmpTrampolineEnd - gSmpTrampoline);
		MemoryFence();

		// calculate the physical addresses
		_Atomic(UINT32) *SmpTplBootedFlag =
			(_Atomic(UINT32) *)((UINT8 *)SmpTplBase +
					    ((UINTN)&gSmpTplBootedFlag -
					     (UINTN)gSmpTrampoline));
		UINT64 *SmpTplInfoStruct =
			(UINT64 *)((UINT8 *)SmpTplBase +
				   ((UINTN)&gSmpTplInfoStruct -
				    (UINTN)gSmpTrampoline));
		IA32_DESCRIPTOR *SmpTplGdt =
			(IA32_DESCRIPTOR *)((UINT8 *)SmpTplBase +
					    ((UINTN)&gSmpTplGdt -
					     (UINTN)gSmpTrampoline));
	...
	for (int i = 0; i < EntryCount; i++) {
		EFI_MEMORY_DESCRIPTOR *Desc =
			(EFI_MEMORY_DESCRIPTOR *)((UINTN)MemoryMap +
						  DescriptorSize * i);
	...
	JumpToStivale2Kernel(Struct, Header.Stack, (void *)Elf.Entry,
			     RequestedPml5);
	UNREACHABLE();

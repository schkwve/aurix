...
EFI_STATUS LoadStivaleKernel(BOOT_ENTRY *Entry)
{
	...
	CHECK_TRACE(Header.GraphicsFramebuffer,
		    "Text mode is not supported in UEFI!");

	// override the gfx info
	INT32 GfxMode = config.GfxMode;
	if (!config.OverrideGfx && Header.FramebufferHeight != 0 &&
	    Header.FramebufferWidth != 0) {
		GfxMode = GetBestGfxMode(Header.FramebufferWidth,
	...
	Struct->Cmdline =
		(UINT64)AllocateReservedPool(StrLen(Entry->Cmdline) + 1);
	...
	if (!EFI_ERROR(EfiGetSystemConfigurationTable(&gEfiAcpi20TableGuid,
		...
		Struct->Rsdp = (UINT64)AllocateReservedCopyPool(36, acpi_table);
			   ...
			   &gEfiAcpi10TableGuid, &acpi_table))) {
	...
	Struct->Epoch = GetUnixEpoch(Time.Second, Time.Minute, Time.Hour,
	...
	for (LIST_ENTRY *Link = Entry->BootModules.ForwardLink;
		...
		STIVALE_MODULE *NewModule =
			AllocateReservedZeroPool(sizeof(STIVALE_MODULE));
	...
	STIVALE_MMAP_ENTRY *StartFrom = AllocateReservedPool(
		(MemoryMapSize / DescriptorSize) * sizeof(STIVALE_MMAP_ENTRY));
	...
	JumpToStivaleKernel(Struct, Header.Stack, (void *)Elf.Entry,
			    Header.Pml5Enable && level5Supported);

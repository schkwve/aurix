...
static void *PushBootParams(void *data, UINTN size)
{
	...
	if (mBootParamsBuffer == NULL) {
		...
		mBootParamsBuffer = AllocateCopyPool(
			mBootParamsSize + AllocationSize, mBootParamsBuffer);
		FreePool(old);
	}
...
LoadMB2Header(EFI_SIMPLE_FILE_SYSTEM_PROTOCOL *fs, CHAR16 *file,
	      UINTN *headerOff)
{
	...
	for (int i = 0; i < MULTIBOOT_SEARCH; i += MULTIBOOT_HEADER_ALIGN) {
		CHECK_AND_RETHROW(
			FileRead(mb2image, &header, sizeof(header), i));

		// check if this is a valid header
		if (header.magic == MULTIBOOT2_HEADER_MAGIC &&
		    header.architecture == MULTIBOOT_ARCHITECTURE_I386 &&
		    (header.checksum + header.magic + header.architecture +
			...
			*headerOff = i;
			...
			CHECK_AND_RETHROW(FileRead(mb2image, ptr,
...
EFI_STATUS LoadMB2Kernel(BOOT_ENTRY *Entry)
{
	...
	// get the header
	struct multiboot_header *header =
		LoadMB2Header(Entry->Fs, Entry->Path, &HeaderOffset);
	CHECK_ERROR_TRACE(header != NULL, EFI_NOT_FOUND,
			  "Could not find a valid multiboot2 header!");
	...
	// iterate the entries
	for (struct multiboot_header_tag *tag =
		     (struct multiboot_header_tag *)(header + 1);
	     tag < (struct multiboot_header_tag *)((UINTN)header +
						   header->header_length) &&
	     tag->type != MULTIBOOT_HEADER_TAG_END;
	     tag = (struct multiboot_header_tag
			    *)((UINTN)tag +
			       ALIGN_VALUE(tag->size, MULTIBOOT_TAG_ALIGN))) {
		switch (tag->type) {
				...
				default:
					CHECK_ERROR_TRACE(
						tag->flags &
							MULTIBOOT_HEADER_TAG_OPTIONAL,
						EFI_UNSUPPORTED,
						"Requested tag %d which is unsupported!",
						r);
				}
			}
			break;
		...
		case MULTIBOOT_HEADER_TAG_ENTRY_ADDRESS: {
			struct multiboot_header_tag_entry_address
				*entry_address = (void *)tag;
			EntryAddressOverride = entry_address->entry_addr;
		} break;

		case MULTIBOOT_HEADER_TAG_CONSOLE_FLAGS: {
			struct multiboot_header_tag_console_flags
				*console_flags = (void *)tag;
			if (console_flags->console_flags &
		...
		case MULTIBOOT_HEADER_TAG_FRAMEBUFFER: {
			...
			if (!config.OverrideGfx && framebuffer->width != 0 &&
			    framebuffer->height != 0) {
				GfxMode = GetBestGfxMode(framebuffer->width,
							 framebuffer->height);
			}
		...
		case MULTIBOOT_HEADER_TAG_EFI_BS: {
			if (!(tag->flags & MULTIBOOT_HEADER_TAG_OPTIONAL)) {
				CHECK_FAIL_TRACE(
					"We do not support passing boot services");
			}
		} break;
		...
		case MULTIBOOT_HEADER_TAG_ENTRY_ADDRESS_EFI32: {
			if (!(tag->flags & MULTIBOOT_HEADER_TAG_OPTIONAL)) {
				CHECK_FAIL_TRACE(
					"We do not support EFI boot for i386");
			}
		} break;
		...
		case MULTIBOOT_HEADER_TAG_ENTRY_ADDRESS_EFI64: {
			if (!(tag->flags & MULTIBOOT_HEADER_TAG_OPTIONAL)) {
				CHECK_FAIL_TRACE(
					"We do not support EFI boot for x86_64");
			}
		} break;
		...
		case MULTIBOOT_HEADER_TAG_RELOCATABLE: {
			if (!(tag->flags & MULTIBOOT_HEADER_TAG_OPTIONAL)) {
				...
				CHECK_FAIL_TRACE(
					"We do not support ELF relocations");
			}
		} break;
	...
	// push the command line
	{
		...
		UINTN size = StrLen(Entry->Cmdline) + 1 +
	...
	for (LIST_ENTRY *Link = Entry->BootModules.ForwardLink;
		...
		UINTN TotalTagSize =
			OFFSET_OF(struct multiboot_tag_module, cmdline) +
			StrLen(Module->Tag) + 1;
		struct multiboot_tag_module *mod =
			PushBootParams(NULL, TotalTagSize);
		...
		TRACE("    Added %s (%s) -> %p - %p", Module->Tag, Module->Path,
	...
	void *acpi10table;
	if (!EFI_ERROR(EfiGetSystemConfigurationTable(&gEfiAcpi10TableGuid,
						      &acpi10table))) {
		// RSDP is 20 bytes long
		TRACE("Pushing old ACPI info");
		struct multiboot_tag_old_acpi *old_acpi = PushBootParams(
			NULL,
			20 + OFFSET_OF(struct multiboot_tag_old_acpi, rsdp));
		old_acpi->size =
			20 + OFFSET_OF(struct multiboot_tag_old_acpi, rsdp);
	...
	if (!EFI_ERROR(EfiGetSystemConfigurationTable(&gEfiAcpi20TableGuid,
		...
		struct multiboot_tag_new_acpi *new_acpi = PushBootParams(
			NULL,
			36 + OFFSET_OF(struct multiboot_tag_new_acpi, rsdp));
		new_acpi->size =
			36 + OFFSET_OF(struct multiboot_tag_new_acpi, rsdp);
	...
	if (NotElf) {
		...
		if (EFI_ERROR(LoadElf32(Entry->Fs, Entry->Path, &elf_info))) {
			...
			CHECK_AND_RETHROW(
				LoadElf64(Entry->Fs, Entry->Path, &elf_info));
		}

		// push elf info
		TRACE("Pushing ELF info");
		UINTN Size =
			OFFSET_OF(struct multiboot_tag_elf_sections, sections) +
			elf_info.SectionHeadersSize;
		...
		sections->num =
			elf_info.SectionHeadersSize / elf_info.SectionEntrySize;
		sections->shndx = elf_info.StringSectionIndex;
		CopyMem(sections->sections, elf_info.SectionHeaders,
			elf_info.SectionHeadersSize);
	...
	UINT8 *start_from = PushBootParams(
		NULL, OFFSET_OF(struct multiboot_tag_efi_mmap, efi_mmap) +
			      MemoryMapSize +
			      OFFSET_OF(struct multiboot_tag_mmap, entries) +
			      MemoryMapSize);
	...
	mmap->size = OFFSET_OF(struct multiboot_tag_mmap, entries) +
		     EntryCount * sizeof(struct multiboot_mmap_entry);
	...
	struct multiboot_tag_efi_mmap *efi_mmap =
		(struct multiboot_tag_efi_mmap *)ALIGN_VALUE(
			(UINTN)mmap + mmap->size, MULTIBOOT_TAG_ALIGN);
	efi_mmap->size = MemoryMapSize +
			 OFFSET_OF(struct multiboot_tag_efi_mmap, efi_mmap);
	...
	struct multiboot_tag *end_tag = (struct multiboot_tag *)ALIGN_VALUE(
		(UINTN)efi_mmap + efi_mmap->size, MULTIBOOT_TAG_ALIGN);

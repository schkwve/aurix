...
EFI_STATUS LoadLinuxKernel(BOOT_ENTRY *Entry)
{
	...
	CHECK_AND_RETHROW(
		LoadBootModule(&Module, (UINTN *)&KernelImage, &KernelSize));
	...
	UINT8 *KernelBuf = LoadLinuxAllocateKernelPages(
		SetupBuf, EFI_SIZE_TO_PAGES(KernelInitialSize));
	...
	if (Entry->Cmdline) {
		...
		CommandLineBuf = LoadLinuxAllocateCommandLinePages(
	...
	if (!IsListEmpty(&Entry->BootModules)) {
		BOOT_MODULE *InitrdModule = BASE_CR(
			Entry->BootModules.ForwardLink, BOOT_MODULE, Link);

		UINT8 *InitrdBase;
		LoadBootModule(InitrdModule, (UINTN *)&InitrdBase, &InitrdSize);
		Print(L"Initrd size: 0x%x\n", InitrdSize);

		InitrdBuf = LoadLinuxAllocateInitrdPages(
			SetupBuf, EFI_SIZE_TO_PAGES(InitrdSize));

...
EFI_STATUS LoadElf32(EFI_SIMPLE_FILE_SYSTEM_PROTOCOL *fs, CHAR16 *file,
		     ELF_INFO *info)
{
	...
	for (int i = 0; i < ehdr.e_phnum; i++) {
		CHECK_AND_RETHROW(
			FileRead(elfFile, &phdr, sizeof(Elf32_Phdr),
		...
		case PT_LOAD: {
			...
			UINTN nPages = EFI_SIZE_TO_PAGES(
				ALIGN_VALUE(phdr.p_memsz, EFI_PAGE_SIZE));

			// allocate the address
			EFI_PHYSICAL_ADDRESS base =
				info->VirtualOffset ?
					phdr.p_vaddr - info->VirtualOffset :
					phdr.p_paddr;
			TRACE("    BASE = %p, PAGES = %d", base, nPages);
			EFI_CHECK(gBS->AllocatePages(
				AllocateAddress, gKernelAndModulesMemoryType,
				nPages, &base));
			CHECK_AND_RETHROW(FileRead(elfFile, (void *)base,
						   phdr.p_filesz,
						   phdr.p_offset));
			ZeroMem((void *)(base + phdr.p_filesz),
	...
	info->SectionHeaders =
		AllocatePool(info->SectionHeadersSize); // TODO: Delete if error
	info->SectionEntrySize = ehdr.e_shentsize;
	info->StringSectionIndex = ehdr.e_shstrndx;
	CHECK_AND_RETHROW(FileRead(elfFile, info->SectionHeaders,
				   info->SectionHeadersSize, ehdr.e_shoff));
